...
func run(runner) -> void:
	...
	runner.assert_eq(Dictionary(fire_events[0].get("hit", {})).get("damage", 0), 12, "fire event base damage")
	...
	runner.assert_eq(Dictionary(talisman_events[0].get("carrier", {})).get("id", ""), "projectile", "talisman fire carrier type")
	runner.assert_eq(Dictionary(talisman_events[0].get("hit", {})).get("splash_radius", 0), 96, "talisman fire level 3 area modifier applies")
	fire_system.free()
	...
	runner.assert_eq(Dictionary(crossbow_events[0].get("carrier", {})).get("pierce", 0), 1, "mechanism crossbow should start with piercing bolts")
	...
	runner.assert_eq(Dictionary(bell_events[0].get("carrier", {})).get("id", ""), "area", "bell should emit an area carrier")
	runner.assert_eq(Dictionary(bell_events[0].get("target", {})).get("range", 0), 220, "bell level 2 range modifier applies")
	runner.assert_eq(Dictionary(bell_events[0].get("hit", {})).get("knockback", 0), 80, "bell event includes knockback")
	bell_system.free()
	...
	if db.has_weapon("spirit_needle_array"):
		...
		if not needle_events.is_empty():
			runner.assert_eq(Dictionary(needle_events[0].get("target", {})).get("id", ""), "radial", "Spirit Needle Array should use radial aiming")
			runner.assert_eq(Dictionary(needle_events[0].get("carrier", {})).get("count", 0), 6, "Spirit Needle Array should start with six needles")
			runner.assert_eq(Dictionary(needle_events[0].get("visual", {})).get("carrier", ""), "res://art/weapons/spirit_needle_array/spirit_needle_projectile.png", "Spirit Needle Array should identify its production projectile")
